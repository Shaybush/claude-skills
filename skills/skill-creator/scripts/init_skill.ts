...
**Appropriate for:** TypeScript/Python scripts, shell scripts, or any executable code that performs automation, data processing, or specific operations.
...
const exampleScript = (skillName: string): string => `#!/usr/bin/env -S npx tsx
/**
...
export function initSkill(skillName: string, path: string): string | null {
  ...
  try {
    ...
    console.log(`❌ Error creating directory: ${(e as Error).message}`);
    return null;
  }
  ...
  try {
    ...
    console.log(`❌ Error creating SKILL.md: ${(e as Error).message}`);
    return null;
  }
  ...
  try {
    ...
    writeFileSync(exampleScriptPath, exampleScript(skillName));
    ...
    writeFileSync(join(referencesDir, 'api_reference.md'), exampleReference(skillTitle));
    console.log('✅ Created references/api_reference.md');

    // Create assets/ directory with example asset placeholder
    const assetsDir = join(skillDir, 'assets');
    mkdirSync(assetsDir, { recursive: true });
    writeFileSync(join(assetsDir, 'example_asset.txt'), exampleAsset);
    console.log('✅ Created assets/example_asset.txt');
  } catch (e) {
    console.log(`❌ Error creating resource directories: ${(e as Error).message}`);
    return null;
  }
